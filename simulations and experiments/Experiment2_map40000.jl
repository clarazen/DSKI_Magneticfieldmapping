...
#
#   the data set used consists of N = xxxxxx measurements recorded by the 
...
#%% make map of the motion capture lab
xtest1 = range(-201.5, 0, 200)
xtest2 = range(-3.5, 5.5, 25)
...
img = load("Figures/3me_long-modified.png");
imgx = range(-206.5, 8, size(img,1));
imgy = range(-12.5, 17.5, size(img,2));

# generate data plot
plt_data = plot(aspect_ratio = :equal, axis = ([], false),legend = false); #  
plot!(imgx, imgy, reverse(img, dims=1), yflip = false, dpi = 1000);
scatter!(X[1,:], X[2,:], zcolor=[norm(y[:,i]) for i=1:size(y,2)],markerstrokewidth=0,colorbar=true,markersize=1,c=cgrad(:default, rev=true));
plot(plt_data)
...
scatter!(Xtest[:,1], Xtest[:,2], zcolor = μcurlmag, markersize = 2, markerstrokewidth = 0, markeralpha = sd, markershape = :square,c=cgrad(:default, rev=true));
plot(plt_map)
savefig(plt_map, "Figures/map40000.png")
